...
function studiorubik_scripts() {
    ...
    wp_enqueue_style('main', get_template_directory_uri() . '/scss/main.min.css', array(), '1.0.0'); //Main Style Sheet


    // JavaScript
    wp_deregister_script('jquery');
    wp_enqueue_script('jquery', 'https://ajax.googleapis.com/ajax/libs/jquery/3.4.1/jquery.min.js', array(), null, true); //jQuery
    wp_enqueue_script('navigationjs', get_template_directory_uri() . '/js/helpers/nav.js', array('jquery'), '1.0.0', true); // Navigation JS
    if( is_front_page() ):
        // Call cube.js only on the Front Page
    wp_enqueue_script('cubejs', get_template_directory_uri() . '/js/helpers/cube.js', array('jquery'), true, '1.0.0', ); // 3D Cube JS
    endif;

    wp_enqueue_script('scripts', get_template_directory_uri() . '/js/main.js', array('jquery', 'navigationjs' ), '1.0.0', true); // Main JS
